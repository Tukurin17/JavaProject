...
public class Aula {
   ...
   /* static void NotasTridimensional2(int nAl){
    ...
    static void nota(int nAl){
        int nTrimestre, nAsignatura;
        nTrimestre=ES.leeN("Numeros de trimestre: ");
        nAsignatura=ES.leeN("Numero de Asignatura: ");
        notas=new int[nAl][nTrimestre][nAsignatura];
        
        for (int i = 0; i < nAl; i++) {
            for (int j = 0; j < nTrimestre; j++) {
                for (int k = 0; k < nAsignatura; k++) {
                    String asignatura=ES.leeDeTeclado("Nombre de la Asignatura");
                    notas[i][j][k]=ES.leeN("Introduce notas del alumno del "+alumnos[i]+" en la eva "+(j+1)+" en la asignatura de "+asignatura);
                    System.out.println(notas[i][j][k]);
                }
            }
        }
